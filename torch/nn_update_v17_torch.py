...
def NN_loop_v17(loops, loop_size, model, NN, convert_in, convert_out, \
    ...
    for loop in range(loops):
        ...
        NN = torch.nn.Sequential(torch.nn.Linear(model['dim'] + 1, 100),
                                torch.nn.ReLU(),
        ...
        optimizer = torch.optim.Adam(NN.parameters(), lr=lr, betas=(b1, b2))

        for epoch in range(epoch_num):
            inputs = torch.Tensor(x_).to(device)
            targets = torch.Tensor(y_).to(device)
            optimizer.zero_grad()
            outputs = NN(inputs)
        ...
        if r_curr > max_check:
            max_check = r_curr
            NN_max = torch.nn.Sequential(torch.nn.Linear(model['dim'] + 1, 100),
                                        torch.nn.ReLU(),
                                        torch.nn.Linear(100, 1))
            NN_max.to(device)
            NN_max.load_state_dict(NN.state_dict())
