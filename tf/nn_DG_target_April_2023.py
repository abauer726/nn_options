...
from stock_v9 import stock_thin
...
np.random.seed(15)
tf.random.set_seed(15)
stock_coa_target_m2 = stock_target_sim(100000, mcall2, target_dt) 

# Thin at coarse dt
coa_dt = 1/3
model_update(mcall2, dt = coa_dt)
stock_coa_m2 = stock_thin(stock_fine_m2, mcall2, coa_dt) # Coarse time grid
stock_target_m2 = stock_target_sim(100000, mcall2, target_dt) 
...
np.random.seed(15)
tf.random.set_seed(15)
(NN_seq_coa_m2, x_m2, y_m2) = NN_seq_train_mt(stock_coa_target_m2, mcall2, c_in_m2, c_out_m2, \
                                theta = 'average', data = True, node_num = 30, \
                                epoch_num = epnum, optim = opt, display_time=False)
...
# Computing the option price
r_seq_target_m2 = NN_payoff_mt(0, stockFwd_target_m2, mcall2, 'seq', NN_seq_coa_m2, c_in_m2, \
                              c_out_m2, display_time=False)
        
print('Stage 1 Price:', np.round(np.mean(r_seq_target_m2), 4))
...
# Validation paths for target dt
target_dt = 1/12     # Target exercise grid
stock_target_m3 = stock_target_sim(10000, mcall3, target_dt) 
...
NN_max_m3 = NN_loop_v28(loops, loop_size, mcall3, NN_agg_m3, c_in_m3, c_out_m3, \
                        target_dt, stock_check = stock_target_m3, epoch_num = epnum_s3, \
                        display_time = True)
...
np.random.seed(18)
tf.random.set_seed(18)
stockFwd_target_m3 = stock_target_sim(100000, mcall3, target_dt) 

## Testing 
# Final Loop Price
r_agg_target_m3 = NN_payoff_target(0, stockFwd_target_m3, mcall3, 'agg', \
            NN_agg_m3, c_in_m3, c_out_m3, target_dt, stop = False, display_time=False)
# Max Loop Price
r_max_target_m3 = NN_payoff_target(0, stockFwd_target_m3, mcall3, 'agg', \
            NN_max_m3, c_in_m3, c_out_m3, target_dt, stop = False, display_time=False)
    
print('Stage 3 -Final- Price:', np.round(np.mean(r_agg_target_m3), 4))
print('Stage 3 -Max- Price:', np.round(np.mean(r_max_target_m3), 4))
...
NN_max_m5 = NN_loop_v28(loops, loop_size, mcall5, NN_agg_m5, c_in_m5, c_out_m5, \
