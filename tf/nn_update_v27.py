...
from stock_v8 import sim_gbm
from stock_v8 import stock_sim
from stock_v8 import stock_sample
...
def NN_loop_v27(loops, loop_size, model, NN, convert_in, convert_out, target, \
    ...
    threshold = 1
    sample = stock_sample(loop_size, model, NN, convert_in, convert_out, threshold)
    ...
    for loop in range(loops):
        ...
        plt.savefig('Hist-Loop-'+str(loop+1)+'-price-'+str(np.round(r_curr, 4))+'.png', dpi=1000)
        plt.clf()
        ...
        for k in range(nSims):
            if times[k] == model['T']:
                ...
                if (cont[-1][k] == False) and (cont[-2][k] == True):
                    vals[k] = imm_pay[k]
                    stop[k] = times[k]
                #elif (cont[-1][k] == False) and (cont[-2][k] == False):
                #    vals[k] = np.exp(model['r']*model['dt'])*vals[k]                    
