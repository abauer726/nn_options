...
def NN_loop_v24(loops, loop_size, model, NN, convert_in, convert_out, \
    ...
    for loop in range(loops):
        ...
        idx0 = np.random.choice(range(loop_size), \
                    size=int(round(loop_size*0.01)), replace=False)
        arr0_time = np.random.choice(range(nSteps-1), \
                    size=int(round(loop_size*0.01)), replace=True)
        ...
        idx1 = np.random.choice(range(len(sample_t_neg)), \
                    size=int(round(loop_size*0.99/2)), replace=False)
