...
def NN_loop_v23(loops, loop_size, model, NN, convert_in, convert_out, \
    ...
    for loop in range(loops):
        ...
        # Merging subsamples
        s0_loc = np.concatenate((arr1_loc, arr2_loc))
        s0_time = np.concatenate((arr1_time, arr2_time))
        ...
        for t_step in np.round(np.arange(model['dt'], model['T'], model['dt']), 4):
            ...
            x = s0[step]
            y = NN_loop_pay_v9(t_step, stock, model, NN, convert_in, convert_out)
...
def NN_loop_pay_v9(t_step, stock, model, NN, convert_in, convert_out, \
